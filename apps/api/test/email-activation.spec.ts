...
describe('Email Activation & User Onboarding Flow', () => {
    ...
    beforeAll(async () => {
        ...
        if (Array.isArray(cookies)) {
            hrToken = cookies.find(c => c.startsWith('token='))?.split(';')[0].split('=')[1] || '';
        } else {
...
${importedEmail},New,Employee,FR,FR`;
    ...
    it('Step 3: Submit Activation Token and Set Password', async () => {
        ...
        const tokenRes = await app.pg.query("SELECT token FROM email_cta_tokens JOIN users ON users.user_id = email_cta_tokens.user_id WHERE users.email = $1 AND action = 'password-reset'", [importedEmail]);
