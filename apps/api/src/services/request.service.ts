  ...
  async createRequest(userId: string, date: string, status: work_status, reason: string): Promise<EntryRequest> {
    ...
    if (user) {
      ...
      await Promise.all(
        hrUsers.map((hr) =>
          this.emailService
            .sendEmail(hr.email, subject, text, `<p>${text.replace(/\n/g, '<br>')}</p>`)
  ...
  async processRequest(
    ...
    if (user) {
      ...
      await this.emailService
        .sendEmail(user.email, subject, text, `<p>${text.replace(/\n/g, '<br>')}</p>`)
        .catch((err) => log.error({ err, email: user.email }, `Failed to email user ${user.email}`));

      // Send push notification
      if (this.pushService) {
        const dateStr =
          typeof request.date === 'object' ? (request.date as Date).toISOString().split('T')[0] : request.date;

        if (action === 'approve') {
          await this.pushService
            .sendRequestApproved(request.user_id, dateStr, adminNote)
            .catch((err) => log.error({ err }, 'Failed to send push notification'));
        } else {
          await this.pushService
            .sendRequestRejected(request.user_id, dateStr, adminNote)
            .catch((err) => log.error({ err }, 'Failed to send push notification'));
        }
      }
    }
