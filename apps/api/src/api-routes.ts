...
export default async function apiRoutes(server: FastifyInstance) {
  ...
  // Push notification service - wire up to request service
  ...
  const authController = new AuthController(authService, userService, securityService);
  const adminController = new AdminController(userService);
  const countryController = new CountryController(countryService);
  const hrController = new HRController(hrService, entryService);
  ...
  server.register(pushTokenRoutes, { pushService });
  server.register(leadRoutes, { prefix: '', controller: leadController });
}
