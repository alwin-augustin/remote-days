...
export class TokenRepository implements ITokenRepository {
  ...
  async findByTokenAndAction(token: string, action: string): Promise<Token | null> {
    const { rows } = await this.pool.query<Token>('SELECT * FROM email_cta_tokens WHERE token = $1 AND action = $2', [
      token,
      action,
    ]);
    return rows[0] || null;
  ...
  async findByToken(token: string): Promise<Token | null> {
    const { rows } = await this.pool.query<Token>('SELECT * FROM email_cta_tokens WHERE token = $1', [token]);
    return rows[0] || null;
  }

  async markAsUsed(token: string): Promise<void> {
    await this.pool.query('UPDATE email_cta_tokens SET used = true WHERE token = $1', [token]);
  }
}
